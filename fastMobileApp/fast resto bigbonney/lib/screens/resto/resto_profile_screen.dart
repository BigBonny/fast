...
class RestoProfileScreen extends StatelessWidget {
  ...
  Widget _buildBase64Image(String data) {
    try {
      ...
      return Image.memory(bytes, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _placeholderImage());
    } catch (_) {
                ...
                children: [
                  settings?.image != null && settings!.image.isNotEmpty
                      ? (settings.image.startsWith('http')
                          ? Image.network(settings.image, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _placeholderImage())
                          : _buildBase64Image(settings.image))
                      : _placeholderImage(),
                  Container(
