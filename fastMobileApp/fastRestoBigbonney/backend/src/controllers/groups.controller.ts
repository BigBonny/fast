...
export const updateHostLocation = async (req: Request, res: Response): Promise<void> => {
  const id = req.params.id as string;
  const { lat, lng, locationName } = req.body as { lat: number; lng: number; locationName?: string };
...
export const getRestaurantGroupOrders = async (req: Request, res: Response): Promise<void> => {
  ...
  const consolidated = groups.map((group) => {
    const allItems = group.orders.flatMap((order) =>
      order.items.map((item) => ({
        ...
        itemName: item.menuItem?.name || 'Article',
        quantity: item.quantity,
        price: item.quantity * (item.menuItem?.price || 0),
        selectedOptions: item.selectedOptions,
      }))
    );
    const total = group.orders.reduce((sum, order) => sum + (order.total || 0), 0);
    const memberCount = group.members.length;
    const paidCount = group.orders.length;
    ...
    return {
      ...
      hostPhone: group.host.phone,
      hostLat: group.hostLat,
      hostLng: group.hostLng,
      hostLocationName: group.hostLocationName,
