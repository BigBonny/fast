...
const quickCategories = [
  { label: "FOOD DROP", subtitle: "Commande express", emoji: "⚡", bg: "linear-gradient(135deg,#dc2626,#b91c1c)", id: "drop", action: "orders" },
  { label: "AVEC MES AMIS", subtitle: "Commande de groupe", emoji: "👥", bg: "linear-gradient(135deg,#7c3aed,#4f46e5)", id: "amis", action: "group" },
  { label: "SURPRISE ME", subtitle: "Laisse-toi surprendre", emoji: "✨", bg: "linear-gradient(135deg,#374151,#1a1f2e)", id: "surprise", action: "surprise" },
  { label: "-20% CODE", subtitle: "Offre du moment", emoji: "🎁", bg: "linear-gradient(135deg,#d97706,#b45309)", id: "promo", action: "promo" },
];

function getCuisineEmoji(type: string) {
  const map: Record<string, string> = {
    burger: "🍔", pizza: "🍕", kebab: "🥙", tacos: "🌮", mexicain: "🌶️",
    africain: "🌍", arabe: "🫓", sushi: "🍣", indien: "🍛", chinois: "🥡",
    thai: "🍜", poulet: "🍗", sandwich: "🥪", hotdog: "🌭", pates: "🍝",
    salade: "🥗", vegan: "🥦", dessert: "🍰", crepes: "🥞", waffle: "🧇",
    cafe: "☕", smoothie: "🥤", autre: "🍴",
  };
  return map[type] || "🍴";
}
...
export default function Home() {
  ...
  const [suggestions, setSuggestions] = useState<{ type: string; label: string; id?: string; cuisine?: string; emoji: string }[]>([]);
  ...
  const handleSearchChange = (value: string) => {
    ...
    const restSuggestions = restaurants
      ...
      .slice(0, 4)
      .map((r: any) => ({ type: "restaurant", label: r.name, id: r.id, emoji: "🍽️" }));

    const cuisineSuggestions = cuisineTypes
      .filter((c) => c.label.toLowerCase().includes(q))
      .slice(0, 3)
      .map((c) => ({ type: "cuisine", label: c.label, cuisine: c.type, emoji: getCuisineEmoji(c.type) }));
      ...
      <div className="relative overflow-hidden" style={{ background: "#08090f" }}>
        ...
        <div className="relative z-10 flex items-center justify-between px-5 pt-5 pb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/cart">
              <button className="w-9 h-9 rounded-xl flex items-center justify-center text-lg"
        ...
        <div className="relative z-20 px-5 pb-5" ref={searchRef}>
